...
$Validator = Join-Path $RepoRoot "ORGANS\THRONE\VALIDATORS\validate_astronomicon_crown_stage_scoring_integration.py"

Write-Host "== ASTRONOMICON-CROWN-STAGE-SCORING-INTEGRATION-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Astronomicon Crown stage scoring integration validation failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\THRONE\TOOLS\__pycache__" -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force "ORGANS\THRONE\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "ASTRONOMICON CROWN STAGE SCORING INTEGRATION PASS"
