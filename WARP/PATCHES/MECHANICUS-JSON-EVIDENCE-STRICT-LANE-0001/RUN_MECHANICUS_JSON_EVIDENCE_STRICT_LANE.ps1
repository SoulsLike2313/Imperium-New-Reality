...
$Validator = Join-Path $RepoRoot "ORGANS\MECHANICUS\VALIDATORS\validate_mechanicus_json_evidence_strict_lane.py"

Write-Host "== MECHANICUS-JSON-EVIDENCE-STRICT-LANE-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Mechanicus JSON evidence strict lane failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\MECHANICUS\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force "ORGANS\MECHANICUS\TOOLS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "MECHANICUS JSON EVIDENCE STRICT LANE PASS"
