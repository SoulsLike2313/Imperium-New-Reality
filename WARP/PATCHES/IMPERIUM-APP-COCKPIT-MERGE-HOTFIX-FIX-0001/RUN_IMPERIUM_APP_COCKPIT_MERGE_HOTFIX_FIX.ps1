...
$Validator = Join-Path $RepoRoot "ORGANS\MECHANICUS\VALIDATORS\validate_imperium_app_cockpit_merge_hotfix_fix.py"

Write-Host "== IMPERIUM-APP-COCKPIT-MERGE-HOTFIX-FIX-0001 =="
...
if ($LASTEXITCODE -ne 0) {
  throw "Imperium App cockpit merge hotfix fix failed with exit code $LASTEXITCODE"
}

Remove-Item -Recurse -Force "ORGANS\MECHANICUS\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "IMPERIUM APP COCKPIT MERGE HOTFIX FIX PASS"
