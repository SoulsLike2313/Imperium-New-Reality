...
$Validator = Join-Path $RepoRoot "ORGANS\ASTRONOMICON\VALIDATORS\validate_imperium_app_tauri_proof_run.py"

Write-Host "== IMPERIUM-APP-TAURI-PROOF-RUN-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Imperium App Tauri proof run failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\ASTRONOMICON\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "IMPERIUM APP TAURI PROOF RUN PASS"
