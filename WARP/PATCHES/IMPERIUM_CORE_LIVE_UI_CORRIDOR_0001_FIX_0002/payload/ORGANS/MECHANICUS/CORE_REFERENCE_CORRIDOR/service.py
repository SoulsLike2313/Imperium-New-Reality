...
from .live_ui_evidence import create_action_context, live_evidence_root
...
class CorridorService:
    def __init__(self, start: Path | str = ".") -> None:
        ...
        # One-call correlation context consumed only by the existing canonical
        # diagnostic return route. It is never persisted directly.
        self._pending_ui_action_context: dict[str, str] | None = None
    ...
    def execute_demo(self) -> dict[str, Any]:
        action_context = self._pending_ui_action_context
        self._pending_ui_action_context = None
        state = self.task_store.load()
        ...
        if action_context is None:
            ...
            evidence_id = action_context["evidence_id"]
            envelope["evidence_id"] = evidence_id
            envelope["event_id"] = action_context["event_id"]
            envelope["action_request_id"] = action_context["action_request_id"]
            envelope["ui_action_id"] = "run_core_diagnostic"
            envelope["ui_dispatch"] = "CORRIDOR_UI_ACTION_CANONICAL_ROUTE"
            store = EvidenceStore(live_evidence_root(self.paths.report))
    ...
    def ui_action(self, action_id: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if action_id == "run_core_diagnostic":
            self._pending_ui_action_context = create_action_context(payload)
