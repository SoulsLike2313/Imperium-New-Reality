...
$Validator = Join-Path $RepoRoot "ORGANS\ASTRONOMICON\VALIDATORS\validate_patch_lifecycle_launcher_commands.py"

Write-Host "== PATCH-PACK-LIFECYCLE-LAUNCHER-COMMANDS-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Patch lifecycle launcher commands validation failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "SUPPORT\LAUNCHER\__pycache__" -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force "ORGANS\ASTRONOMICON\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "PATCH LIFECYCLE LAUNCHER COMMANDS PASS"
