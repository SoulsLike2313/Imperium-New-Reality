...
$Validator = Join-Path $RepoRoot "ORGANS/MECHANICUS/VALIDATORS/validate_mechanicus_tool_admission_v2.py"

Write-Host "== MECHANICUS-TOOL-ADMISSION-V2-0001 =="
...
if ($LASTEXITCODE -ne 0) {
  throw "Mechanicus tool admission V2 failed with exit code $LASTEXITCODE"
}

Remove-Item -Recurse -Force (Join-Path $RepoRoot "ORGANS/MECHANICUS/VALIDATORS/__pycache__") -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force (Join-Path $RepoRoot "ORGANS/MECHANICUS/TOOLS/__pycache__") -ErrorAction SilentlyContinue

Write-Host "MECHANICUS TOOL ADMISSION V2 PASS"
