...
$Validator = Join-Path $RepoRoot "ORGANS\MECHANICUS\VALIDATORS\validate_mechanicus_task_tool_composition_planner_ultrasafe_hotfix.py"

Write-Host "== MECHANICUS-TASK-TOOL-COMPOSITION-PLANNER-ULTRASAFE-HOTFIX-0001 =="
...
if ($LASTEXITCODE -ne 0) {
  throw "Mechanicus task tool composition planner ultrasafe hotfix failed with exit code $LASTEXITCODE"
}

Remove-Item -Recurse -Force "ORGANS\MECHANICUS\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force "ORGANS\MECHANICUS\TOOLS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "MECHANICUS TASK TOOL COMPOSITION PLANNER ULTRASAFE HOTFIX PASS"
