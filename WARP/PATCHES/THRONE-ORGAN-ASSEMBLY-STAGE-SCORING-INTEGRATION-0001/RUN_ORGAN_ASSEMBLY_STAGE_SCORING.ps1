...
$Validator = Join-Path $RepoRoot "ORGANS\THRONE\VALIDATORS\validate_organ_assembly_stage_scoring.py"

Write-Host "== THRONE-ORGAN-ASSEMBLY-STAGE-SCORING-INTEGRATION-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Organ assembly stage scoring validation failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\THRONE\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "THRONE ORGAN ASSEMBLY STAGE SCORING PASS"
