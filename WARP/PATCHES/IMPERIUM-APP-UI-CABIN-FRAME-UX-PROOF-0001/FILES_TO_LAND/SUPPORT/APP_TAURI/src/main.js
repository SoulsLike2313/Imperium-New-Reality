...
function markUx(actionName) {
  uxActionCount += 1;
  logAquarium("UX", `${UX_PROOF_MARKER} ${CABIN_FRAME_UX_PROOF} action=${actionName} count=${uxActionCount}`);
}

function logAquarium(kind, message) {
  const line = `[${nowStamp()}][${kind}] ${message}`;
  aquariumLines = [...aquariumLines.slice(-120), line];
  render();
}
...
async function registerPatchPack() {
  ...
  const response = await callAnyCommand(["register_patch_pack", "register_warp_patch_pack", "register_patch"], { patchId: selectedPatchId, patch_id: selectedPatchId });
...
async function loadLanguagePowers() {
  ...
  const response = await callAnyCommand(["load_language_power_codex", "load_language_powers", "get_language_powers", "mechanicus_language_powers"]);
  if (response.ok) {
  ...
  return `<section class="room-panel" data-marker="${PATCH_FORGE_ROOM}">
    ...
    <div class="table-wrap">
      <table>
        ...
        <tbody>
          ${patchPacks.map((p) => `<tr><td>${p.status}</td><td>${p.patch_id}</td><td>${p.runner || "—"}</td></tr>`).join("")}
        </tbody>
      </table>
    </div>
  </section>`;
...
function render() {
  ...
  app.innerHTML = `
    <main class="app-shell" data-marker="${IMPERIUM_APP_PLATFORM}" data-ui-target="${IMPERIUM_APP_UI_CABIN_FRAME}">
      <header class="hero">
        ...
        <div class="hud">
          <span><i>◎</i><b>FPS</b><em id="fps-readout">60</em><small>/ target ${FPS_LOCK_TARGET}</small></span>
          <span><i>▣</i><b>Repo</b><em>E:\\IMPERIUM_REALITY</em></span>
          <span><i>✥</i><b>Marker</b><em>${IMPERIUM_TAURI_SHELL}</em></span>
          <span><i>✦</i><b>App</b><em>${APP_COCKPIT_MERGED_INTO_PLATFORM}</em></span>
        </div>
      </header>
  ...
  document.querySelector("#clear-aquarium")?.addEventListener("click", () => {
    uxActionCount += 1;
    aquariumLines = [`[${nowStamp()}][UX] ${UX_PROOF_MARKER} ${CABIN_FRAME_UX_PROOF} action=clear_aquarium_log count=${uxActionCount}`];
    render();
  });
    ...
    if (lastFrame > 0) {
      ...
      if (!runtimeFpsProofSent && frameDeltas.length >= warmupFrames + minimumSampleCount) {
        ...
        recordRuntimeFpsProof({
          ...
          ui_target_marker: IMPERIUM_APP_UI_CABIN_FRAME,
...
aquariumLines = [
  `[${nowStamp()}][AUTH] Imperium App Platform cabin frame awakened`,
  `[${nowStamp()}][STYLE] ${IMPERIUM_APP_UI_CABIN_FRAME}: gothic metal / cyber glow / trash-polka accents`,
  `[${nowStamp()}][LAW] UI renders truth; core receipts prove truth`,
  `[${nowStamp()}][UX] ${UX_PROOF_MARKER} ${CABIN_FRAME_UX_PROOF} action=initial_render count=0`
];
