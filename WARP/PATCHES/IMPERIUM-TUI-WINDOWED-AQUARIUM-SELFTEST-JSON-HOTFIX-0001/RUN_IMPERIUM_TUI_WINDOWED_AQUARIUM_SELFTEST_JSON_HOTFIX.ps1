...
$Validator = Join-Path $RepoRoot "ORGANS\ASTRONOMICON\VALIDATORS\validate_imperium_tui_windowed_aquarium_launcher.py"

Write-Host "== IMPERIUM-TUI-WINDOWED-AQUARIUM-SELFTEST-JSON-HOTFIX-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Imperium TUI windowed aquarium selftest JSON hotfix failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\ASTRONOMICON\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "IMPERIUM TUI WINDOWED AQUARIUM SELFTEST JSON HOTFIX PASS"
