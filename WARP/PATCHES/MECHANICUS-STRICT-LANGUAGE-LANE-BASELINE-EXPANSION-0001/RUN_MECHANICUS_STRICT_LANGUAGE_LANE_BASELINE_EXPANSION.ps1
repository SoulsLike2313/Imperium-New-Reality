...
$Validator = Join-Path $RepoRoot "ORGANS\MECHANICUS\VALIDATORS\validate_mechanicus_strict_language_lane_baseline_expansion.py"

Write-Host "== MECHANICUS-STRICT-LANGUAGE-LANE-BASELINE-EXPANSION-0001 =="
...
if ($LASTEXITCODE -ne 0) {
  throw "Mechanicus strict language lane baseline expansion failed with exit code $LASTEXITCODE"
}

Remove-Item -Recurse -Force "ORGANS\MECHANICUS\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force "ORGANS\MECHANICUS\TOOLS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "MECHANICUS STRICT LANGUAGE LANE BASELINE EXPANSION PASS"
