...
$Validator = Join-Path $RepoRoot "ORGANS\MECHANICUS\VALIDATORS\validate_mechanicus_strict_build_lane_runner_exit_code_fix.py"

Write-Host "== MECHANICUS-STRICT-BUILD-LANE-RUNNER-EXIT-CODE-FIX-0001 =="
...
if ($LASTEXITCODE -ne 0) {
  throw "Mechanicus strict build lane runner exit-code fix failed with exit code $LASTEXITCODE"
}

Remove-Item -Recurse -Force "ORGANS\MECHANICUS\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force "ORGANS\MECHANICUS\TOOLS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "MECHANICUS STRICT BUILD LANE RUNNER EXIT CODE FIX PASS"
