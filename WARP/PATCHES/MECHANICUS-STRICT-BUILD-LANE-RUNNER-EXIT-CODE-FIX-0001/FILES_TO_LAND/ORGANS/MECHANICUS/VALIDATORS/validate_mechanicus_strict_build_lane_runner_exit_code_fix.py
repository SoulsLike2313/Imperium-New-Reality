...
PATCHER = Path("ORGANS/MECHANICUS/TOOLS/patch_mechanicus_strict_build_lane_runner_exit_code.py")
...
PATCH_REPORT = Path("ORGANS/MECHANICUS/REPORTS/MECHANICUS_STRICT_BUILD_LANE_RUNNER_EXIT_CODE_PATCH_REPORT_V0_1.json")

RECEIPT = Path("ORGANS/MECHANICUS/RECEIPTS/mechanicus_strict_build_lane_runner_exit_code_fix_receipt.json")
SUMMARY = Path("ORGANS/MECHANICUS/REPORTS/MECHANICUS_STRICT_BUILD_LANE_RUNNER_EXIT_CODE_FIX_SUMMARY_V0_1.json")
REPORT_MD = Path("ORGANS/MECHANICUS/REPORTS/MECHANICUS_STRICT_BUILD_LANE_RUNNER_EXIT_CODE_FIX_REPORT_V0_1.md")
...
def main() -> int:
    ...
    patcher_ok = (repo / PATCHER).is_file()
    add(checks, "runner_exit_code_patcher_installed", patcher_ok, {"path": PATCHER.as_posix()})
    if not patcher_ok:
        errors.append("patcher missing")

    if not errors:
        patch_run = run_py(repo, PATCHER, ["--repo-root", str(repo)], timeout=120)
        patch_data, patch_err = load_json(repo / PATCH_REPORT) if (repo / PATCH_REPORT).is_file() else ({}, "missing")
        patch_ok = patch_run["exit_code"] == 0 and patch_err is None and patch_data.get("verdict", "").startswith("PASS")
        add(checks, "runner_exit_code_patch_applies_cleanly", patch_ok, {"run": patch_run, "patch_report": patch_data, "load_error": patch_err})
        if not patch_ok:
            errors.append("runner exit-code patch did not apply cleanly")
    ...
    markers_ok = (
        ...
        and 'report.get("verdict") == "PASS_MECHANICUS_STRICT_BUILD_LANE_FOUNDATION"' in runner_text
    )
    add(checks, "runner_contains_v0_2_exit_code_and_legacy_validator_markers", markers_ok, {"path": RUNNER.as_posix()})
    if not markers_ok:
        errors.append("runner markers incomplete after patch")
    ...
    if isinstance(plan, dict):
        ...
        if isinstance(rec, dict):
            warnings.append(f"Planner recommended demand after runner fix: {rec.get('demand_id')} score={rec.get('score_0_to_100')} verdict={rec.get('verdict')}")
        for m in plan.get("missing_capabilities", [])[:6]:
            warnings.append(f"Remaining planner gap: {m.get('capability_id')} => {m.get('severity')}")

    verdict = "PASS_MECHANICUS_STRICT_BUILD_LANE_RUNNER_EXIT_CODE_FIX_READY" if not errors else "FAIL_MECHANICUS_STRICT_BUILD_LANE_RUNNER_EXIT_CODE_FIX"
    generated = utc()
    summary = {
        "summary_id": "mechanicus.strict_build_lane_runner_exit_code_fix_summary.v0_1",
        "task_id": TASK_ID,
        ...
        "plan": PLAN.as_posix(),
        "meaning": "Strict build runner exit code now follows report truth; false-negative guard warning should be gone."
    }
    receipt = {
        "receipt_id": "receipt.mechanicus.strict_build_lane_runner_exit_code_fix.v0_1",
        "task_id": TASK_ID,
        "validator_id": VALIDATOR_ID,
        "verdict": verdict,
        "generated_at_utc": generated,
    ...
    (repo / REPORT_MD).write_text(f"""# MECHANICUS STRICT BUILD LANE RUNNER EXIT CODE FIX REPORT V0.1
