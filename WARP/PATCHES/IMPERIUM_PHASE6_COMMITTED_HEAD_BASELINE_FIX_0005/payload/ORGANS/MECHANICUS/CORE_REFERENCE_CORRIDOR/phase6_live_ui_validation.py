...
def _windows_path_identity(value: str | Path) -> str:
    """Normalize equivalent Win32 spellings without using PATH resolution."""
    raw = str(value).strip()
...
def _same_executable_identity(observed: str | Path, registered: str | Path) -> bool:
    observed_path = Path(str(observed))
    registered_path = Path(str(registered))
    try:
        if observed_path.is_file() and registered_path.is_file():
            return os.path.samefile(observed_path, registered_path)
    except (OSError, ValueError):
        pass
    return _windows_path_identity(observed) == _windows_path_identity(registered)
...
def verify_after(repo: Path, reality: Path, report: Path, baseline_path: Path, receipt_path: Path, proof_path: Path) -> dict[str, Any]:
    ...
    for capability_id, key in (("CORE_GIT", "git"), ("CORE_PWSH", "pwsh")):
        ...
        observed_path_raw = str(observed.get("executable", ""))
        capability_path_raw = str(capability.get("executable_path", ""))
        if not _same_executable_identity(observed_path_raw, capability_path_raw):
            raise RuntimeError(
                f"live diagnostic {key} path differs from registry: "
                f"observed={observed_path_raw!r}, registered={capability_path_raw!r}"
            )
    ...
    receipt = {
        ...
        "tool_path_identity_rule": "OS_SAMEFILE_OR_WINDOWS_EXTENDED_PATH_NORMALIZATION",
