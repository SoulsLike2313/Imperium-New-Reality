...
$Validator = Join-Path $RepoRoot "ORGANS\ASTRONOMICON\VALIDATORS\validate_imperium_app_tauri_shell_frontend_marker_hotfix.py"

Write-Host "== IMPERIUM-APP-TAURI-SHELL-FRONTEND-MARKER-HOTFIX-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Imperium App Tauri shell frontend marker hotfix failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\ASTRONOMICON\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "IMPERIUM APP TAURI SHELL FRONTEND MARKER HOTFIX PASS"
