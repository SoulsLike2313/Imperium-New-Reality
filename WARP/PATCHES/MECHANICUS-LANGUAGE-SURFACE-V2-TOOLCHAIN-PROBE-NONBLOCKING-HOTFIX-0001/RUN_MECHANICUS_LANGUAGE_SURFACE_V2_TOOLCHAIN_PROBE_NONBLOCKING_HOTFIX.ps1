...
$Validator = Join-Path $RepoRoot "ORGANS\MECHANICUS\VALIDATORS\validate_mechanicus_language_surface_v2_toolchain_probe_nonblocking_hotfix.py"

Write-Host "== MECHANICUS-LANGUAGE-SURFACE-V2-TOOLCHAIN-PROBE-NONBLOCKING-HOTFIX-0001 =="
...
if ($LASTEXITCODE -ne 0) {
  throw "Mechanicus language surface v2 toolchain probe nonblocking hotfix failed with exit code $LASTEXITCODE"
}

Remove-Item -Recurse -Force "ORGANS\MECHANICUS\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force "ORGANS\MECHANICUS\TOOLS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "MECHANICUS LANGUAGE SURFACE V2 TOOLCHAIN PROBE NONBLOCKING HOTFIX PASS"
