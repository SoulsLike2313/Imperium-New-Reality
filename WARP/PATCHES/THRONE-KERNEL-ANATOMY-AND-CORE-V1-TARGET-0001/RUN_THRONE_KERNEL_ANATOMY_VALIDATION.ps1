...
$Validator = Join-Path $RepoRoot "ORGANS\THRONE\VALIDATORS\validate_throne_kernel_anatomy.py"

Write-Host "== THRONE-KERNEL-ANATOMY-AND-CORE-V1-TARGET-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Throne kernel anatomy validator failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\THRONE\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "THRONE KERNEL ANATOMY VALIDATION PASS"
