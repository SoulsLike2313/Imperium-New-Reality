...
Write-Host "== THRONE-MECHANICUS-SIX-GATE-CROWN-VERDICT-0001 =="
Write-Host "Repo root: $RepoRoot"
Write-Host "Files to land: $FilesToLand"

$Validator = Join-Path $RepoRoot "ORGANS/THRONE/VALIDATORS/validate_throne_mechanicus_six_gate_crown_verdict.py"
if (-not (Test-Path $Validator)) {
  $ValidatorSource = Join-Path $FilesToLand "ORGANS/THRONE/VALIDATORS/validate_throne_mechanicus_six_gate_crown_verdict.py"
...
if ($LASTEXITCODE -ne 0) {
  throw "Throne Mechanicus six-gate crown verdict failed with exit code $LASTEXITCODE"
}

Write-Host "THRONE MECHANICUS SIX GATE CROWN VERDICT PASS" -ForegroundColor Green
