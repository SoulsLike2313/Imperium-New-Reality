...
const PINNED_TOOL_ENV_KEYS: [&str; 5] = [
    "IMPERIUM_PINNED_TOOLCHAIN_REQUIRED",
...
pub(crate) fn add_pinned_tool_environment(
    environment: &mut MinimalEnvironment,
    path_key: &str,
    hash_key: &str,
    executable: &Path,
    sha256: &str,
) -> Result<(), String> {
    if !PINNED_TOOL_ENV_KEYS.contains(&path_key) || !PINNED_TOOL_ENV_KEYS.contains(&hash_key) {
        return Err("BRIDGE_PINNED_ENV_KEY_NOT_ADMITTED".to_string());
    }
    if !executable.is_absolute() || !executable.is_file() {
        return Err("BRIDGE_PINNED_EXECUTABLE_NOT_ABSOLUTE_FILE".to_string());
    }
    if sha256.len() != 64 || !sha256.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return Err("BRIDGE_PINNED_EXECUTABLE_HASH_INVALID".to_string());
    }
    environment.insert(OsString::from(path_key), executable.as_os_str().to_os_string());
    environment.insert(OsString::from(hash_key), OsString::from(sha256));
    Ok(())
}
