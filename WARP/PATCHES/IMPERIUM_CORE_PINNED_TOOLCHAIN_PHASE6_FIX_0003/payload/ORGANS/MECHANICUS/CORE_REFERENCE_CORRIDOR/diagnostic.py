...
from .pinned_tools import git_argv, pinned_git_executable, pinned_pwsh_executable, sha256_file
...
def _git(root: Path, *args: str) -> str:
    return _run(git_argv("-C", str(root), *args), root)
...
def collect_diagnostic(start: Path | str = ".") -> dict[str, Any]:
    ...
    pwsh_path = pinned_pwsh_executable()
    pwsh_version = _run(
    ...
    return {
        ...
        "toolchain": {
            "git": {
                ...
                "sha256": sha256_file(git_path),
                "path_resolution_used": False,
            },
            "pwsh": {
                "executable": str(pwsh_path),
                "sha256": sha256_file(pwsh_path),
                "path_resolution_used": False,
            },
        },
        "powershell": {
