...
def _process_alive(pid: int) -> bool:
    if pid <= 0:
        return False
    if os.name == "nt":
        # tasklist emits bytes using the active Windows console/OEM code page.
        # Never decode localized output: PID digits and CSV quotes are ASCII-stable.
        result = subprocess.run(
        ...
        return result.returncode == 0 and f'"{pid}"'.encode("ascii") in stdout
    try:
        os.kill(pid, 0)
