...
$Validator = Join-Path $RepoRoot "ORGANS\MECHANICUS\VALIDATORS\validate_mechanicus_ui_workshop_no_monolith_matrix_hotfix.py"

Write-Host "== MECHANICUS-UI-WORKSHOP-NO-MONOLITH-MATRIX-HOTFIX-0001 =="
...
if ($LASTEXITCODE -ne 0) {
  throw "Mechanicus UI workshop no-monolith matrix hotfix failed with exit code $LASTEXITCODE"
}

Remove-Item -Recurse -Force "ORGANS\MECHANICUS\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force "ORGANS\MECHANICUS\TOOLS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "MECHANICUS UI WORKSHOP NO-MONOLITH MATRIX HOTFIX PASS"
