...
$Validator = Join-Path $RepoRoot "ORGANS\MECHANICUS\VALIDATORS\validate_imperium_app_ui_cabin_frame_ux_proof_fix.py"

Write-Host "== IMPERIUM-APP-UI-CABIN-FRAME-UX-PROOF-FIX-0001 =="
...
if ($LASTEXITCODE -ne 0) {
  throw "Imperium App UI cabin frame UX proof fix failed with exit code $LASTEXITCODE"
}

Remove-Item -Recurse -Force "ORGANS\MECHANICUS\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "IMPERIUM APP UI CABIN FRAME UX PROOF FIX PASS"
