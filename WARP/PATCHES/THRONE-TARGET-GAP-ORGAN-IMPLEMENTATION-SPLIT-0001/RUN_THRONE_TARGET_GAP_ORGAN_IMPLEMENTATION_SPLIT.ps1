...
Write-Host "== THRONE-TARGET-GAP-ORGAN-IMPLEMENTATION-SPLIT-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Throne target gap organ implementation split failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\THRONE\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "THRONE TARGET GAP ORGAN IMPLEMENTATION SPLIT PASS"
