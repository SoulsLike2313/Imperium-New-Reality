...
#[derive(Debug, Serialize, Deserialize)]
struct RuntimeFpsProofResult {
    receipt: String,
    fps_lock_proven: bool,
    average_fps: f64,
    target_fps: f64,
}
...
#[tauri::command]
fn record_runtime_fps_proof(app: tauri::AppHandle, payload: Value) -> Result<RuntimeFpsProofResult, String> {
    let root = repo_root(&app)?;
    let stamp = Utc::now().format("%Y%m%d_%H%M%S").to_string();
    let receipt_path = root
        .join("SUPPORT/APP_TAURI/receipts")
        .join(format!("{stamp}_runtime_fps_proof_receipt.json"));

    let target_fps = payload.get("target_fps").and_then(|v| v.as_f64()).unwrap_or(60.0);
    let average_fps = payload.get("average_fps").and_then(|v| v.as_f64()).unwrap_or(0.0);
    let sample_count = payload.get("sample_count").and_then(|v| v.as_u64()).unwrap_or(0);
    let slow_frame_ratio = payload.get("slow_frame_ratio").and_then(|v| v.as_f64()).unwrap_or(1.0);
    let max_slow_frame_ratio = 0.05_f64;
    let average_threshold = 59.5_f64;
    let fps_lock_proven = average_fps >= average_threshold && sample_count >= 180 && slow_frame_ratio <= max_slow_frame_ratio;

    let verdict = if fps_lock_proven {
    ...
    let receipt = json!({
        "receipt_id": format!("receipt.tauri.runtime_fps_proof.{stamp}"),
        "verdict": verdict,
        "proof_id": "RUNTIME_FPS_PROOF",
        "generated_at_utc": Utc::now().to_rfc3339(),
        "fps_lock_proven": fps_lock_proven,
        "target_fps": target_fps,
        "average_fps_pass_threshold": average_threshold,
        "max_slow_frame_ratio": max_slow_frame_ratio,
        "payload": payload,
        "errors": if fps_lock_proven { json!([]) } else { json!(["runtime fps lock not proven"]) },
        "warnings": []
    });
    write_json(&receipt_path, &receipt)?;

    Ok(RuntimeFpsProofResult {
