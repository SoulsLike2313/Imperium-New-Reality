...
$Validator = Join-Path $RepoRoot "ORGANS\ASTRONOMICON\VALIDATORS\validate_imperium_app_tauri_runtime_window_fps_proof.py"

Write-Host "== IMPERIUM-APP-TAURI-RUNTIME-WINDOW-FPS-PROOF-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Imperium App Tauri runtime window FPS proof failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\ASTRONOMICON\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "IMPERIUM APP TAURI RUNTIME WINDOW FPS PROOF PASS"
