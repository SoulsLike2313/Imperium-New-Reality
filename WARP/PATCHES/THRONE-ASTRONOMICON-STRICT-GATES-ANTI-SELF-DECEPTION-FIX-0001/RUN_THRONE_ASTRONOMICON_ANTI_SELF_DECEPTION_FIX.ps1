...
$Validator = Join-Path $RepoRoot "ORGANS\THRONE\VALIDATORS\validate_throne_astronomicon_anti_self_deception.py"

Write-Host "== THRONE-ASTRONOMICON-STRICT-GATES-ANTI-SELF-DECEPTION-FIX-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Throne Astronomicon anti self-deception fix failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\THRONE\TOOLS\__pycache__" -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force "ORGANS\THRONE\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "THRONE ASTRONOMICON ANTI SELF-DECEPTION PASS"
