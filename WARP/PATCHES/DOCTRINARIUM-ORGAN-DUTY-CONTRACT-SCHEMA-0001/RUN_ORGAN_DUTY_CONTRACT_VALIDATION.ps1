...
$Validator = Join-Path $RepoRoot "ORGANS\DOCTRINARIUM\VALIDATORS\validate_organ_duty_contract_coverage.py"

Write-Host "== DOCTRINARIUM-ORGAN-DUTY-CONTRACT-SCHEMA-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Organ duty contract validation failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\DOCTRINARIUM\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "DOCTRINARIUM ORGAN DUTY CONTRACT SCHEMA PASS"
