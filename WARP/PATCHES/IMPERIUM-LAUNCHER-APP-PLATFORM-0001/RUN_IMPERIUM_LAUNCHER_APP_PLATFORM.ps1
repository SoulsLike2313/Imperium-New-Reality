...
$Validator = Join-Path $RepoRoot "ORGANS\ASTRONOMICON\VALIDATORS\validate_imperium_launcher_app_platform.py"

Write-Host "== IMPERIUM-LAUNCHER-APP-PLATFORM-0001 =="
...
if ($LASTEXITCODE -ne 0) { throw "Imperium Launcher app platform validation failed with exit code $LASTEXITCODE" }

Remove-Item -Recurse -Force "ORGANS\ASTRONOMICON\VALIDATORS\__pycache__" -ErrorAction SilentlyContinue

Write-Host "IMPERIUM LAUNCHER APP PLATFORM PASS"
