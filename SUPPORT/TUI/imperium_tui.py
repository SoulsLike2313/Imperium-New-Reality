...
def internal_status(repo: Path, sink) -> int:
    ...
    table_pairs("Astronomicon / Custodes / Throne", [
        ...
        ("External witness for Throne", throne.get("external_witness_for_throne_score")),
...
def internal_throne_readout(repo: Path, sink) -> int:
    ...
    table_pairs("Трон: последний Crown-приговор по Астрономикону", [
        ...
        ("astronomicon_throne_confirmed_score", data.get("astronomicon_throne_confirmed_score")),
...
def command_is_forbidden(cmd: List[str]) -> str | None:
    lowered = [str(x).lower() for x in cmd]
    pairs = [("git", "commit"), ("git", "push")]
    for a, b in pairs:
...
def run_external(repo: Path, action: Dict[str, Any], sink) -> int:
    ...
    forbidden = command_is_forbidden(cmd)
    if forbidden:
        print_line(f"ЗАПРЕЩЕНО TUI: {forbidden}", sink)
        return 2
...
def run_action(repo: Path, action_id: str) -> int:
    ...
    if action.get("aquarium_log_required") is not True:
    ...
    # Two markers: Russian for the owner, ASCII for validators and future IDE parsers.
    print("")
    print(f"Лог аквариума: {log_path.relative_to(repo)}")
    print(f"AQUARIUM_LOG: {log_path.relative_to(repo)}")
    print(f"Receipt TUI: {receipt_path.relative_to(repo)}")
    print(f"TUI_RECEIPT: {receipt_path.relative_to(repo)}")
    return code

def list_actions(repo: Path) -> int:
    data = load_json(repo / ACTIONS_FILE)
    if not isinstance(data, dict):
        print(f"Не найден manifest TUI: {ACTIONS_FILE}")
        return 2
