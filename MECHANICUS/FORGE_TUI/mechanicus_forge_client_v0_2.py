...
def repo_root() -> Path:
    # <repo>/IMPERIUM_NEW_GENERATION/MECHANICUS/FORGE_TUI/mechanicus_forge_client_v0_2.py
...
def evidence_metrics() -> dict[str, str]:
    ...
    records = (
        ...
        or raw.get("record_count")
        or raw.get("records")
        or raw.get("total_records")
        or "440"
    )
    return {
        "records": str(records),
...
class MechanicusForgeClient(App):
    ...
    TITLE = "Mechanicus Forge Client V0.2"
    ...
    def on_mount(self) -> None:
        ...
        self.selected_category = "UTILITIES"
    ...
    def update_truth_strip(self) -> None:
        ...
        panel = Panel(
            grid,
            title=f"[bold {BRASS}]MECHANICUS FORGE CLIENT V0.2 — ARSENAL REGISTRY SURFACE[/]",
    ...
    def update_folder_tree(self) -> None:
        ...
        for row in folder_freshness():
            ...
            root.add_leaf(f"{row['path']} | {row['files']} files | {status} | {row['fresh']}", data=row["path"])
        root.expand()
    ...
    def filtered_cards(self) -> list[dict[str, Any]]:
        ...
        return sorted(
            [c for c in self.cards if c["category"] == self.selected_category],
    ...
    def update_status_summary(self) -> None:
        ...
        self.query_one("#status_summary", Static).update(
            Panel(table, title=f"[bold {BRASS}]REGISTRY SUMMARY[/]", subtitle=f"[cyan]Selected: {self.selected_category}[/]", border_style=COPPER)
        )
    ...
    def update_detail(self, card: dict[str, Any]) -> None:
        ...
        for receipt in self.matching_receipts(card):
            receipt_table.add_row(receipt["time"], f"{receipt['file_name']} [{receipt['status']}]")
        if not self.matching_receipts(card):
        ...
        self.query_one("#detail_panel", Static).update(
            Panel(group, title=f"[bold {BRASS}]CAPABILITY DETAIL[/]", border_style=COPPER)
        )
    ...
    def update_receipts(self) -> None:
        ...
        self.query_one("#receipt_panel", Static).update(
            Panel(table, title=f"[bold {BRASS}]LATEST RECEIPTS (PROVENANCE)[/]", border_style=COPPER)
        )
    ...
    def update_actions(self) -> None:
        ...
        self.query_one("#action_panel", Static).update(
            Panel(table, title=f"[bold {BRASS}]ACTION SOCKETS[/]", border_style=COPPER)
        )
