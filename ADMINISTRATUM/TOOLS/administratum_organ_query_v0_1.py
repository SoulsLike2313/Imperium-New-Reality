...
def build_response(task_id: str, question: str, mode: str) -> Dict[str, Any]:
    ...
    if "owner verdict" in lowered or "scope exception" in lowered:
        verdict = "OWNER_VERDICT_NEEDED"
        owner_question = "Разрешает ли Owner исключение из текущих scope/hygiene ограничений?"
    elif "skip receipt" in lowered or "ignore status" in lowered:
        verdict = "BLOCK"
    ...
    payload = build_verdict_payload(
        ...
        not_proven=[
            "full automated archive intelligence",
            "autonomous stage orchestration",
        ],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Wave1 Administratum organ query tool.")
    parser.add_argument("--task-id", default=TASK_ID_DEFAULT)
    parser.add_argument("--question", default="What registration/history/continuity hygiene is required?")
...
def main() -> int:
    ...
    if args.sample:
        question = "What registration/history/continuity hygiene is required?"
