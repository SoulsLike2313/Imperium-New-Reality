#!/usr/bin/env python3
"""Administratum task report bundle packager v0.2."""

from __future__ import annotations

import argparse
import hashlib
import json
import sys
import zipfile
from datetime import datetime, timezone
from pathlib import Path
from typing import Any


if str(Path(__file__).resolve().parent) not in sys.path:
    sys.path.insert(0, str(Path(__file__).resolve().parent))

from administratum_bundle_gate_v0_2 import (  # noqa: E402
    ACTIVE_ROOT_DEFAULT,
    ADJACENT_MANIFEST,
    ANCIENT_ROOT_DEFAULT,
    DIGEST_RECEIPT,
    MATRIX_VERSION,
    MISSING_REQUEST,
    SCHEMA_BLOCK,
    SCHEMA_PASS,
    SCHEMA_VALIDATION_RECEIPT,
    TASK_ID_DEFAULT,
    V02_GATE_RECEIPT,
    ensure_under,
    find_repo_root,
    sha256_file,
    to_posix,
    validate_report,
    write_gate_outputs,
    write_json,
)


BUNDLE_NAME = "task_report_bundle.zip"
BUNDLE_PACKAGER_RECEIPT = "bundle_packager_receipt.json"
INVENTORY_NAME = "bundle_file_inventory.json"
SHA256SUMS_NAME = "sha256sums.txt"
SELF_REFERENCE_LIMIT = "SELF_REFERENCE_LIMIT.md"
PACKAGING_PASS = "BUNDLE_PACKAGING_PASS"
PACKAGING_BLOCK = "BUNDLE_PACKAGING_BLOCK"

SELF_REFERENCE_ADJACENT = (
    {
        "path": BUNDLE_NAME,
        "class_ids": [],
        "reason": "The bundle cannot include itself.",
        "status": "ADJACENT_SELF_REFERENCE_LIMIT",
    },
    {
        "path": SHA256SUMS_NAME,
        "class_ids": ["sha256_sums"],
        "reason": "Final sums record the final bundle hash and remain adjacent.",
        "status": "ADJACENT_SELF_REFERENCE_LIMIT",
    },
    {
        "path": INVENTORY_NAME,
        "class_ids": ["bundle_manifest_and_file_inventory"],
        "reason": "Inventory records the final included file set and remains adjacent.",
        "status": "ADJACENT_SELF_REFERENCE_LIMIT",
    },
    {
        "path": V02_GATE_RECEIPT,
        "class_ids": ["administratum_composition_receipt"],
        "reason": "Gate receipt records the final bundle hash and remains adjacent.",
        "status": "ADJACENT_SELF_REFERENCE_LIMIT",
    },
    {
        "path": BUNDLE_PACKAGER_RECEIPT,
        "class_ids": [],
        "reason": "Packager receipt records the final bundle hash and remains adjacent.",
        "status": "ADJACENT_SELF_REFERENCE_LIMIT",
    },
    {
        "path": "commit_chain_receipt.json",
        "class_ids": ["commit_chain_identifiers"],
        "reason": "Final commit identifiers have a natural self-reference limit.",
        "status": "ADJACENT_SELF_REFERENCE_LIMIT",
    },
    {
        "path": "git_closure_receipt.json",
        "class_ids": [
            "commit_chain_identifiers",
            "git_closure_and_remote_closure_proof",
            "worktree_clean_or_explicit_cap_receipt",
        ],
        "reason": "Final clean-worktree proof is verified after push without rewriting committed receipts.",
        "status": "ADJACENT_SELF_REFERENCE_LIMIT",
    },
    {
        "path": "remote_head_proof_receipt.json",
        "class_ids": ["git_closure_and_remote_closure_proof"],
        "reason": "Exact origin/master equality is verified after push without rewriting committed receipts.",
        "status": "ADJACENT_SELF_REFERENCE_LIMIT",
    },
    {
        "path": "remote_closure_receipt.json",
        "class_ids": ["git_closure_and_remote_closure_proof"],
        "reason": "Remote closure receipt is adjacent because final push proof is live-verified after commit.",
        "status": "ADJACENT_SELF_REFERENCE_LIMIT",
    },
)

EXCLUDED_FROM_ZIP = {
    BUNDLE_NAME,
    SHA256SUMS_NAME,
    INVENTORY_NAME,
    V02_GATE_RECEIPT,
    BUNDLE_PACKAGER_RECEIPT,
    "commit_chain_receipt.json",
    "git_closure_receipt.json",
    "remote_head_proof_receipt.json",
    "remote_closure_receipt.json",
    SELF_REFERENCE_LIMIT,
    MISSING_REQUEST,
}


def utc_now() -> str:
    return datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")


def write_text(path: Path, text: str) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(text.rstrip() + "\n", encoding="utf-8")


def ensure_adjacent_manifest(report_dir: Path, task_id: str) -> Path:
    payload = {
        "manifest_type": "adjacent_receipts_manifest",
        "task_id": task_id,
        "matrix_version": MATRIX_VERSION,
        "timestamp_utc": utc_now(),
        "reason": "Self-reference-limited final proof files are stored next to the final bundle.",
        "adjacent_files": list(SELF_REFERENCE_ADJACENT),
    }
    path = report_dir / ADJACENT_MANIFEST
    write_json(path, payload)
    return path


def ensure_self_reference_limit(report_dir: Path, task_id: str) -> Path:
    text = f"""# Self Reference Limit

Task ID: `{task_id}`

`task_report_bundle.zip`, final hash files, and final git/remote proof receipts
cannot all be both inside the bundle and also prove the exact final bundle
commit. Administratum V0.2 records these files in `adjacent_receipts_manifest.json`
and verifies the bundle gate before creating the zip.

Final `origin/master == HEAD` and clean-worktree proof must be live-checked after
the last push without rewriting this report folder.
"""
    path = report_dir / SELF_REFERENCE_LIMIT
    write_text(path, text)
    return path


def collect_included_files(report_dir: Path) -> list[Path]:
    files: list[Path] = []
    for path in report_dir.rglob("*"):
        if not path.is_file():
            continue
        rel_path = path.relative_to(report_dir).as_posix()
        if rel_path in EXCLUDED_FROM_ZIP:
            continue
        files.append(path)
    return sorted(files, key=lambda item: item.relative_to(report_dir).as_posix())


def adjacent_entries_with_hashes(report_dir: Path) -> list[dict[str, Any]]:
    entries: list[dict[str, Any]] = []
    for entry in SELF_REFERENCE_ADJACENT:
        row = dict(entry)
        path = report_dir / str(row["path"])
        row["exists"] = path.is_file()
        row["sha256"] = sha256_file(path) if path.is_file() and path.name != SHA256SUMS_NAME else ""
        entries.append(row)
    return entries


def inventory_payload(
    report_dir: Path,
    included_files: list[Path],
    *,
    task_id: str,
    bundle_path: Path,
    bundle_sha256: str = "",
) -> dict[str, Any]:
    return {
        "inventory_type": "task_report_bundle_file_inventory",
        "task_id": task_id,
        "timestamp_utc": utc_now(),
        "report_dir": to_posix(report_dir),
        "bundle_path": to_posix(bundle_path),
        "bundle_sha256": bundle_sha256,
        "included_file_count": len(included_files),
        "included_files": [
            {
                "path": file_path.relative_to(report_dir).as_posix(),
                "size_bytes": file_path.stat().st_size,
                "sha256": sha256_file(file_path),
            }
            for file_path in included_files
        ],
        "adjacent_self_reference_files": adjacent_entries_with_hashes(report_dir),
        "excluded_from_bundle_due_self_reference": sorted(EXCLUDED_FROM_ZIP),
        "authority_boundary": "composition_schema_only",
    }


def write_inventory(report_dir: Path, included_files: list[Path], *, task_id: str, bundle_path: Path, bundle_sha256: str = "") -> Path:
    path = report_dir / INVENTORY_NAME
    write_json(path, inventory_payload(report_dir, included_files, task_id=task_id, bundle_path=bundle_path, bundle_sha256=bundle_sha256))
    return path


def write_sha256sums(report_dir: Path, included_files: list[Path], bundle_path: Path | None = None) -> Path:
    sha_paths: list[Path] = list(included_files)
    for entry in SELF_REFERENCE_ADJACENT:
        path = report_dir / str(entry["path"])
        if path.is_file() and path.name != SHA256SUMS_NAME:
            sha_paths.append(path)
    if bundle_path is not None and bundle_path.is_file():
        sha_paths.append(bundle_path)
    seen: set[str] = set()
    lines: list[str] = []
    for path in sorted(sha_paths, key=lambda item: item.relative_to(report_dir).as_posix() if report_dir in item.resolve().parents else item.name):
        rel_path = path.relative_to(report_dir).as_posix() if report_dir in path.resolve().parents else path.name
        if rel_path in seen:
            continue
        seen.add(rel_path)
        lines.append(f"{sha256_file(path)}  {rel_path}")
    sha_path = report_dir / SHA256SUMS_NAME
    write_text(sha_path, "\n".join(lines))
    return sha_path


def build_zip(report_dir: Path, bundle_path: Path, included_files: list[Path]) -> dict[str, Any]:
    if bundle_path.exists():
        bundle_path.unlink()
    with zipfile.ZipFile(bundle_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        for file_path in included_files:
            archive.write(file_path, file_path.relative_to(report_dir).as_posix())
    return {
        "bundle_path": to_posix(bundle_path),
        "bundle_sha256": sha256_file(bundle_path),
        "bundle_size_bytes": bundle_path.stat().st_size,
        "included_file_count": len(included_files),
        "included_files": [path.relative_to(report_dir).as_posix() for path in included_files],
    }


def write_packager_receipt(
    path: Path,
    *,
    task_id: str,
    verdict: str,
    gate_result: dict[str, Any],
    bundle: dict[str, Any] | None,
) -> dict[str, Any]:
    receipt = {
        "task_id": task_id,
        "verdict": verdict,
        "gate_verdict_before_packaging": gate_result["verdict"],
        "composition_gate": gate_result["composition_gate"],
        "schema_gate": gate_result["schema_gate"],
        "bundle_created": bundle is not None,
        "bundle": bundle,
        "authority_boundary": "composition_schema_only",
        "created_utc": utc_now(),
    }
    write_json(path, receipt)
    return receipt


def package_report(report_dir: Path, *, task_id: str, bundle_name: str = BUNDLE_NAME) -> dict[str, Any]:
    repo_root = find_repo_root(report_dir)
    report_dir = ensure_under(repo_root, report_dir)
    bundle_path = ensure_under(repo_root, report_dir / bundle_name)
    gate_receipt_path = ensure_under(repo_root, report_dir / V02_GATE_RECEIPT)
    schema_receipt_path = ensure_under(repo_root, report_dir / SCHEMA_VALIDATION_RECEIPT)
    digest_path = ensure_under(repo_root, report_dir / DIGEST_RECEIPT)
    missing_path = ensure_under(repo_root, report_dir / MISSING_REQUEST)
    packager_receipt_path = ensure_under(repo_root, report_dir / BUNDLE_PACKAGER_RECEIPT)

    ensure_adjacent_manifest(report_dir, task_id)
    ensure_self_reference_limit(report_dir, task_id)

    included_files = collect_included_files(report_dir)
    write_inventory(report_dir, included_files, task_id=task_id, bundle_path=bundle_path)
    write_sha256sums(report_dir, included_files)

    gate_result = validate_report(
        report_dir,
        task_id=task_id,
        active_root=ACTIVE_ROOT_DEFAULT,
        ancient_root=ANCIENT_ROOT_DEFAULT,
        receipt_path=gate_receipt_path,
    )
    if gate_result["verdict"] == SCHEMA_BLOCK:
        if bundle_path.exists():
            bundle_path.unlink()
        gate_receipt = write_gate_outputs(
            gate_result,
            receipt_path=gate_receipt_path,
            schema_receipt_path=schema_receipt_path,
            digest_path=digest_path,
            missing_request_path=missing_path,
            bundle_created=False,
            bundle_path=bundle_path,
        )
        packager_receipt = write_packager_receipt(
            packager_receipt_path,
            task_id=task_id,
            verdict=PACKAGING_BLOCK,
            gate_result=gate_receipt,
            bundle=None,
        )
        return {
            "verdict": PACKAGING_BLOCK,
            "bundle_created": False,
            "bundle_path": to_posix(bundle_path),
            "missing_items_request_path": to_posix(missing_path),
            "gate_receipt": gate_receipt,
            "packager_receipt": packager_receipt,
        }

    bundle = build_zip(report_dir, bundle_path, included_files)
    write_inventory(report_dir, included_files, task_id=task_id, bundle_path=bundle_path, bundle_sha256=bundle["bundle_sha256"])
    write_sha256sums(report_dir, included_files, bundle_path)

    final_gate_result = validate_report(
        report_dir,
        task_id=task_id,
        active_root=ACTIVE_ROOT_DEFAULT,
        ancient_root=ANCIENT_ROOT_DEFAULT,
        receipt_path=gate_receipt_path,
    )
    gate_receipt = write_gate_outputs(
        final_gate_result,
        receipt_path=gate_receipt_path,
        schema_receipt_path=schema_receipt_path,
        digest_path=digest_path,
        missing_request_path=missing_path,
        bundle_created=True,
        bundle_path=bundle_path,
        bundle_sha256=bundle["bundle_sha256"],
    )
    packager_receipt = write_packager_receipt(
        packager_receipt_path,
        task_id=task_id,
        verdict=PACKAGING_PASS if gate_receipt["verdict"] == SCHEMA_PASS else PACKAGING_BLOCK,
        gate_result=gate_receipt,
        bundle=bundle if gate_receipt["verdict"] == SCHEMA_PASS else None,
    )
    write_inventory(report_dir, included_files, task_id=task_id, bundle_path=bundle_path, bundle_sha256=bundle["bundle_sha256"])
    write_sha256sums(report_dir, included_files, bundle_path)
    if gate_receipt["verdict"] != SCHEMA_PASS:
        if bundle_path.exists():
            bundle_path.unlink()
        return {
            "verdict": PACKAGING_BLOCK,
            "bundle_created": False,
            "bundle_path": to_posix(bundle_path),
            "gate_receipt": gate_receipt,
            "packager_receipt": packager_receipt,
        }
    return {
        "verdict": PACKAGING_PASS,
        "bundle_created": True,
        "bundle": bundle,
        "gate_receipt_path": to_posix(gate_receipt_path),
        "schema_receipt_path": to_posix(schema_receipt_path),
        "digest_path": to_posix(digest_path),
        "inventory_path": to_posix(report_dir / INVENTORY_NAME),
        "sha256sums_path": to_posix(report_dir / SHA256SUMS_NAME),
        "adjacent_manifest_path": to_posix(report_dir / ADJACENT_MANIFEST),
        "packager_receipt": packager_receipt,
    }


def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Administratum task report bundle packager v0.2.")
    parser.add_argument("--report-dir", required=True, help="Task report directory to package.")
    parser.add_argument("--task-id", default=TASK_ID_DEFAULT, help="Task ID for receipts.")
    parser.add_argument("--bundle-name", default=BUNDLE_NAME, help="Bundle filename.")
    parser.add_argument("--result-out", default="", help="Optional path for the packager result receipt.")
    return parser.parse_args(argv)


def main(argv: list[str]) -> int:
    args = parse_args(argv)
    result = package_report(Path(args.report_dir), task_id=args.task_id, bundle_name=args.bundle_name)
    if args.result_out:
        repo_root = find_repo_root(Path(args.report_dir))
        write_json(ensure_under(repo_root, Path(args.result_out)), result)
    print(json.dumps(result, ensure_ascii=True, indent=2))
    return 0 if result["verdict"] == PACKAGING_PASS else 2


if __name__ == "__main__":
    raise SystemExit(main(sys.argv[1:]))
