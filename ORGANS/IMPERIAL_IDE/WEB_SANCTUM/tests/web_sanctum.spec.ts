...
test('web sanctum v0.8.1 opens full stage loop surface', async ({ page }) => {
  await ready(page);
  await expect(page.getByRole('heading', { name: 'IMPERIAL SANCTUM' })).toBeVisible();
  await expect(page.getByTestId('version-pill')).toContainText(/V0.8.1/);
