...
test('collect owner-visible Web Sanctum v0.8.3 screenshots', async ({ page }) => {
  ...
  for (const [name, nav, panel] of pages) {
    ...
    await page.screenshot({ path: `${screenshotRoot}/screenshots/v083-data-atlas-${name}.png`, fullPage: true });
  }
});
