...
async function run() {
  ...
  const brainCoreDetected = (await page.locator("#brainCoreZone").count()) > 0;
  const peripheralStats = await page.evaluate(() => {
    const container = document.querySelector("#brainZoneMap");
    const cards = Array.from(document.querySelectorAll("article.organ-card.brain-zone"));
    if (!container || !cards.length) {
      return {
        container_found: Boolean(container),
    ...
    const inCenter = (nx, ny) => nx >= 0.33 && nx <= 0.67 && ny >= 0.33 && ny <= 0.67;
    const positions = cards.map((card) => {
      const r = card.getBoundingClientRect();
      const cx = r.left + r.width / 2;
      const cy = r.top + r.height / 2;
      const nx = (cx - rect.left) / Math.max(rect.width, 1);
      const ny = (cy - rect.top) / Math.max(rect.height, 1);
      const central = inCenter(nx, ny);
      return {
        organ_id: card.getAttribute("data-organ-id") || "",
        normalized_x: Number(nx.toFixed(3)),
        normalized_y: Number(ny.toFixed(3)),
        central,
      };
    });
    const centralCount = positions.filter((item) => item.central).length;
    return {
      container_found: true,
      card_count: cards.length,
      central_count: centralCount,
      peripheral_count: positions.length - centralCount,
      card_positions: positions,
    };
  });
  await shot("active_brain_or_organ_zone", brainLikeDetected ? "#brainZoneMap" : "#organGrid");
  await shot("brain_core_zone", "#brainZoneMap");
  ...
  audit.truth_checks.push({
    id: "brain_core_zone_detected",
    pass: brainCoreDetected,
    detail: brainCoreDetected ? "#brainCoreZone found" : "#brainCoreZone missing",
  });
  audit.truth_checks.push({
    id: "organ_cards_count_expected_10",
    pass: peripheralStats.card_count === 10,
    card_count: peripheralStats.card_count,
  });
  audit.truth_checks.push({
    id: "organ_cards_peripheral_distribution",
    pass: peripheralStats.container_found && peripheralStats.central_count <= 2,
    container_found: peripheralStats.container_found,
    central_count: peripheralStats.central_count,
    peripheral_count: peripheralStats.peripheral_count,
    positions: peripheralStats.card_positions,
  });

  audit.notes.push(`health_status=${safeText(healthBefore?.status)}`);
  audit.notes.push(`brain_like_detected=${brainLikeDetected}`);
  audit.notes.push(`brain_core_detected=${brainCoreDetected}`);
  audit.notes.push(`central_cards=${peripheralStats.central_count}`);
  ...
  mdLines.push(`- brain_core_detected: ${brainCoreDetected}`);
  mdLines.push(`- peripheral_distribution: central_cards=${peripheralStats.central_count}, peripheral_cards=${peripheralStats.peripheral_count}`);
  mdLines.push("");
